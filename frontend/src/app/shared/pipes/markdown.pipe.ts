...
export class MarkdownPipe implements PipeTransform {
  ...
  transform(text: string): SafeHtml {
    ...
    html = html.replace(/^####\s+(.+)$/gm, '<h4 class="text-xl font-semibold mt-4 mb-2 text-brand-primary">$1</h4>');
    html = html.replace(/^###\s+(.+)$/gm, '<h3 class="text-2xl font-bold mt-5 mb-3 text-brand-primary">$1</h3>');
    ...
    // Inline code (`code`)
    html = html.replace(/`([^`]+)`/g, '<code class="bg-base-200 text-brand-primary px-1.5 py-0.5 rounded text-sm">$1</code>');

    // Links ([text](url))
    html = html.replace(/\[([^\]]+)\]\(([^)]+)\)/g, '<a href="$2" class="text-brand-primary hover:underline" target="_blank" rel="noopener noreferrer">$1</a>');
